...
const {loadSpreadsheet, localesPath, ns, lngs, sheetId, columnKeyToHeader, NOT_AVAILABLE_CELL} = require('./index.ts');
...
async function updateJsonFromSheet() {
  ...
  fs.readdir(localesPath, (error, lngs) => {
    ...
    lngs = lngs.filter((lng) => !lng.includes('.json'));
    lngs.forEach((lng) => {
      const localeJsonFilePath = `${localesPath}/${lng}.json`;
      console.log('sibal', localeJsonFilePath)
      const jsonString = JSON.stringify(lngsMap[lng], null, 2);
      console.log('sibal', jsonString)
